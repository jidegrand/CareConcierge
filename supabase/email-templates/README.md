...
- The **Invite user** template wraps `{{ .ConfirmationURL }}` in an app-owned
  `/set-password?confirmation_url=...` link. This prevents email provider
  prefetch/scanning from consuming the one-time invite before the user clicks
  **Accept invite** in the app.
...
| Invite user | `You're invited to Care Concierge` | `invite.html` | Primary invite template; wraps `{{ .ConfirmationURL }}` |
